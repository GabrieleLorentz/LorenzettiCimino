...
fact proposed{
	always all i: Internship, s: Student | i.InternshipStatus = Registration and s.status = NonInterested
...
fact Interview_Compiled{
	always all s: Student, i: Internship | (s.status = Allowed or s.status = Selected or s.status = RejAfterForm) and s in i.ProposedStudents 
		implies s in i.InterviewToCompile.InterviewCompiledBy
	always all s: Student, i: Internship | (s.status != Allowed and s.status != Selected and s.status != RejAfterForm) 
		implies s !in i.InterviewToCompile.InterviewCompiledBy
}
...
fact Half_Work_Done{
	always all s: Student, i: Internship | (i.InternshipStatus = ISMiddle or i.InternshipStatus = ISFinished) and s.status = Selected and s in i.ProposedStudents 
		implies (s in i.MiddleFeedToCompile.MiddleFeedCompiledBy and i.OwnedBy in i.MiddleFeedToCompile.MFCBY_Company)
	always all s: Student, i: Internship | (i.InternshipStatus != ISMiddle and i.InternshipStatus != ISFinished) 
		implies (s !in i.MiddleFeedToCompile.MiddleFeedCompiledBy and i.OwnedBy !in i.MiddleFeedToCompile.MFCBY_Company)
}
...
fact students_who_have_finished{
	always all s: Student, i: Internship | i.InternshipStatus = ISFinished and s.status = Selected and s in i.ProposedStudents 
		implies (s in i.EndFeedToCompile.EndFeedCompiledBy and s in i.ReviewToCompile.ReviewCompiledBy and 
				i.OwnedBy in i.EndFeedToCompile.EFCBY_Company and i.OwnedBy in i.ReviewToCompile.RCBY_Company)
...
fact STU{
	always all s: Student | s.status = NonInterested 
		implies after (s.status = Applied or  s.status = Recommended or s.status = NonInterested)
